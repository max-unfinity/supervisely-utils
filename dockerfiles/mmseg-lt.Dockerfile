...
RUN git clone --depth 1 https://github.com/open-mmlab/mmdetection.git /mmdetection && \
    cd /mmdetection && \
    pip install --no-cache-dir -e . && \
    rm -rf /mmdetection/.git

# Patch mmdet to accept mmcv 2.2.0 (change < to <=)
RUN sed -i 's/mmcv_version < digit_version/mmcv_version <= digit_version/g' \
    /mmdetection/mmdet/__init__.py && \
    echo "Patched mmdet version check"
...
# Copy mmcv source (needed for editable install)git
COPY --from=builder /mmcv /mmcv
COPY --from=builder /mmdetection /mmdetection
...
RUN mkdir /weights && \
    ...
    wget https://download.openmmlab.com/mmsegmentation/v0.5/mask2former/mask2former_swin-t_8xb2-160k_ade20k-512x512/mask2former_swin-t_8xb2-160k_ade20k-512x512_20221203_234230-7d64e5dd.pth -P /weights

# Install git
RUN apt-get update && apt-get install -y git && apt-get clean && rm -rf /var/lib/apt/lists/*

# Install supervisely and dependencies
RUN pip install --no-cache-dir \
    plotly==5.22.0 \
    markdown \
